'use strict';
fetch('https://mdn.github.io/learning-area/javascript/apis/fetching-data/can-store/products.json').then((response)=> response.json()).then((jsonData)=>console.log('data is ',jsonData)).catch((error)=>(`got this error ${error}`)).finally(()=>console.log(`Data fetching is completed`));
const products = [{
    id: 1,
    name: 'Gaming Laptop',
    price: 1500,
...
function renderCart() {
  ...
  if(cart.length>0)
    {
        const total_price = getTotalPriceFromCart();
    }
}
